...
public class AreaCodeServiceImpl implements AreaCodeService {
	...
	public List<String> getSubAreaNames(int areaCode) {
		...
		ResponseEntity<String> response = restClient.get().uri(SUB_AREA_API_URL, openApiServiceKey, areaCode)
				.accept(MediaType.APPLICATION_JSON).retrieve().toEntity(String.class);

		JsonParser parser = new GsonJsonParser();
		List<Map<String, Object>> items = (List<Map<String, Object>>) ((Map<String, Object>) ((Map<String, Object>) ((Map<String, Object>) parser
				.parseMap(response.getBody()).get("response")).get("body")).get("items")).get("item");
